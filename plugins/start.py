...
@Bot.on_message(filters.command("start") & filters.private & filters.regex('start⚡️') & subsall & subsch & subsgc)
async def start_command(client: Bot, message: Message):
    ...
    if message.text.startswith("/start token_") and VERIFY:
        ...
        try:
            ...
            if int(ad_msg.split(":")[1]) < get_current_time():
                ...
                return
            if int(ad_msg.split(":")[1]) > int(get_current_time() + 26600):
                await client.send_message(
                    message.chat.id,
    ...
    if uid not in ADMINS:
        ...
        if result is None:
            ...
            ad_code = str_to_b64(f"{uid}:{str(get_current_time() + 21600)}")
            ...
            temp_msg = await message.reply("Please wait...")
            ad_code = str_to_b64(f"{uid}:{str(get_current_time() + 26600)}")
            ad_url = shorten_url(f"https://telegram.dog/{client.username}?start=token_{ad_code}")
            await client.send_message(
                message.chat.id,
